...
_INJECTION_PATTERNS: list[re.Pattern[str]] = [
    ...
    re.compile(r"forget\s+(all\s+)?(previous|above|prior|earlier|your)\s+(instructions?|rules?|context)", re.I),
    re.compile(r"override\s+(your\s+)?(previous\s+)?(instructions?|rules?|constraints?|prompt)", re.I),
    re.compile(r"bypass\s+(your\s+)?(safety|security|restrictions?|constraints?|rules?|guidelines?)", re.I),
    ...
    re.compile(r"\bas\s+a\s+(medical\s+student|doctor|nurse|researcher|test|demo)\b.{0,60}(no\s+restrictions?|without\s+guidelines?|pretend)", re.I),
    ...
    re.compile(r"\byou\s+(must|should|will)\s+(now\s+)?(answer|respond|reply)\s+without\s+(restrictions?|guidelines?|rules?)", re.I),
    re.compile(r"\b(must|should|will)\s+now\s+(answer|respond|reply)\s+without\b", re.I),
    re.compile(r"\bnow\s+(answer|respond|reply)\s+without\s+(restrictions?|guidelines?|rules?)", re.I),
    re.compile(r"\banswer\s+without\s+(any\s+)?(restrictions?|guidelines?|rules?|constraints?)\b", re.I),
