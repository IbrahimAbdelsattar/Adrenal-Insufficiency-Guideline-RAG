...
type MainView = "chat" | "inspector";

export default function Page() {
  const [viewMode, setViewMode] = useState<MainView>("chat");
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_22rem] items-start">
      ...
      <div className="min-w-0 space-y-6">
        ...
        {/* ---------------------------------------------------- */}
        {/* Navigation Mode Switcher: Chatbot vs Inspector       */}
        {/* ---------------------------------------------------- */}
        <div className="mono-card flex flex-wrap items-center justify-between gap-3 rounded-2xl p-2.5">
          <div className="mono-inset flex items-center gap-1 rounded-xl p-1 w-full sm:w-auto">
            <button
              type="button"
              onClick={() => setViewMode("chat")}
              className={`flex-1 sm:flex-initial rounded-lg px-4 py-2 text-xs font-extrabold transition-all cursor-pointer ${
                viewMode === "chat"
                  ? "mono-button-primary text-white shadow-sm"
                  : "mono-button text-ink-faint hover:text-ink"
              }`}
            >
              {t.chatTab || "💬 RAG Chatbot"}
            </button>
            <button
              type="button"
            ...
            >
              {t.inspectorTab || "🔍 Retrieval Inspector"}
            </button>
          </div>

          <div className="flex items-center gap-2 text-[11px] text-ink-dim px-2">
            <span className="h-2 w-2 rounded-full bg-accent-bright animate-pulse" />
            <span className="font-semibold">{t.groundedInNice || "Strictly Grounded in NICE NG243"}</span>
          </div>
        </div>

        {viewMode === "chat" ? (
          <ChatView lang={lang} topK={topK} onTopKChange={setTopK} />
        ) : (
          <>
            {/* ---------------------------------------------------- */}
            ...
            {loading && (
              <div className="space-y-4" aria-busy="true">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="mono-card h-44 rounded-2xl p-6 skeleton-shimmer" />
                ))}
              </div>
            )}
                ...
                {isOutOfScope ? (
                  <div className="mono-card rounded-2xl border-caution/40 bg-caution/5 p-6 text-caution animate-fade-in-up">
                    <div className="flex items-center gap-2 font-bold text-sm">
                      ...
                      <span>
                        {lang === "ar" ? "السؤال خارج نطاق Eva AI" : "Question Outside Scope"}
                      </span>
                    </div>
                    ...
                    <div className="mt-4 mono-inset rounded-xl p-4 text-xs text-ink-dim">
                      <p className="font-bold text-ink">
                        {lang === "ar" ? "النطاق الحالي" : "Current Scope"}
                      </p>
            ...
            {!loading && !response && !genResponse && !error && (
              <div className="mono-card rounded-2xl p-10 text-center animate-fade-in-up">
                ...
                <h2 className="mt-5 text-lg font-extrabold text-ink tracking-wide">
                  ...
                  <span className="font-brand-serif text-accent-bright me-2">AI</span>
                ...
                <div className="mt-8 grid grid-cols-1 gap-4 text-left sm:grid-cols-3">
                  ...
                  <FeatureCard icon="🛡️" title={t.feat3Title} desc={t.feat3Desc} />
                </div>
              </div>
            )}
          </>
